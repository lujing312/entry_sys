...
<script>
import {
  getInheritCategoryList,
  getInheritList,
  getInheritInfo,
  addInheritCategory,
  updateInheritCategory,
  rankInheritCategory,
  deleteInheritCategory,
  getSecondInheritCategoryList,
  addSecondInheritCategory,
  addInheritPhenotype,
  updateInheritPhenotype,
  rankInheritPhenotype,
  deleteInheritPhenotype,
  addInheritLevel,
  rankInheritLevel,
  updateInheritLevel,
  updateInheritLevelORColor,
  deleteInheritLevel,
  getInheritPreviewArtcle,
  getAllInheritSecondCategoryList,
  updateInherittypeSecondCategory
} from '../server'
import {
  moveArray
} from '../common/utils'
...
export default {
  ...
  methods: {
    getCategoryList () {
      getInheritCategoryList({}).then(res => {
        if (!res.status) {
    ...
    handleRankCategory (index, type) {
      ...
      // 请求
      rankInheritCategory({categoryList}).then(res => {
        if (!res.status) {
          this.$message({
            type: 'success',
            message: '分类排序成功'
    ...
    handleDeleteSecondCategory (id) {
      ...
      this.$confirm('此操作将删除分类, 请谨慎操作?', '提示', {
        ...
        // 请求删除
        deleteInheritCategory({id}).then(res => {
          if (!res.status) {
            this.$message({
              type: 'success',
              message: '分类删除成功'
    ...
    handleRankPheno (index, type) {
      ...
      rankInheritPhenotype({phenoList}).then(res => {
        if (!res.status) {
          this.$message({
            type: 'success',
            message: '表型排序成功'
    ...
    handlePreviewArticle (levelInfo) {
      ...
      getInheritPreviewArtcle(params).then(res => {
        if (!res.status) {
