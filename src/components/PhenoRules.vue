<template>
  <el-container>
          ...
          <el-form label-position="left" inline class="col-table-expand col-table-expand-small">
            ...
            <el-form-item label="PMID">
              ...
              <el-button
                ...
                circle
                @click="handleGetPmidList(props.row.id, props.row.type)"
              />
            </el-form-item>
    ...
    <el-dialog
      ...
      <p class='link'>链接：<span class="pmid-dialog-text">{{pmidInfo.link}}</span></p>
    ...
    <el-dialog
      title="PMID"
      :visible.sync="handleOpenPmidListDialog"
      width="25%"
      class="pmid-select-dialog">
        <el-autocomplete
          v-model="state4"
        ...
        <el-tag
          ...
          @click.native="handleGetPMID(tag)">{{tag}}</el-tag>
        <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click.native.prevent="handleupdatePmidList">确 定</el-button>
      </span>
    </el-dialog>
  </el-container>
</template>
...
export default {
  ...
  data () {
    return {
      ...
      handleOpenPmidListDialog: false,
      weightPercent: '',
      editRsInfoIndex: -1,
  ...
  methods: {
    ...
    handleGetPmidList (id, type) {
      this.id = id
      this.type = type
      this.getPmidArr()
      this.handleOpenPmidListDialog = true
    },
    getPmidArr () {
      getPmidArr({
        id: this.id,
    ...
    handleupdatePmidList () {
      updatePmidList({
        id: this.id,
        type: this.type,
        pmidList: this.operatePmid
      }).then(res => {
        if (!res.status) {
          this.$message({
            type: 'success',
          ...
          this.handleOpenPmidListDialog = false
          // 更新列表
          this.getPmidArr()
        } else {
          this.$message({
